...
let HomePage = () => {
    ...
    let {getRootProps, getInputProps, isDragActive} = useDropzone({
        onDrop: (acceptedFiles) => {
            acceptedFiles.forEach(({path, name, size, type}) =>
                window.uploadFile({nickname: user.nickname, filePath: path, fileName: name, fileType: type, fileSize: size})
            );
        },
    });
    ...
    return (
        <div
            className="flex flex-col w-screen h-screen select-none text-gray-500 dark:bg-black dark:text-white outline-none">
            {user.connected ? (
                <>
                    <div
                        className="flex flex-row justify-between items-center p-2 bg-gray-100 border-b border-gray-200 dark:bg-black dark:border-gray-800">
            ...
            <div
                className="flex flex-row justify-center items-center p-2 bg-gray-100 border-t border-gray-200 dark:bg-black dark:border-gray-800">
